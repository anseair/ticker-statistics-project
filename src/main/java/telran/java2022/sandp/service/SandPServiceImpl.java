...
public class SandPServiceImpl implements SandPService, CommandLineRunner {
	...
	public SandPStatDto getStat(long periodDays, double sum, long termDays) {
		...
		int end = sandpPeriods.indexOf(new SandP(new SandPDate("sandp", dateEnd), 0.0));
		...
		for (int start = 0; start < end; start++) {
			...
			SandP sandpEnd = null;
			while (!repository.existsById(new SandPDate("sandp", dateEnd1))) {
				dateEnd1 = dateEnd1.minusDays(1);
			}
			sandpEnd = repository.findById(new SandPDate("sandp", dateEnd1)).get();
...
//	===================== VARIANT 2: add in list by stream ======================
	...
	public void run(String... args) throws Exception {
		...
		List<SandP> newDatas = SandPCsvParsing.parsingWithApache("sandp5years.csv", "sandp", "MM/dd/yyyy", 1);

		System.out.println("amount of data sandp in csv: " + newDatas.size());
